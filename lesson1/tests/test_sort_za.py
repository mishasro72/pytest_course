# автотест для проверки сортировки Z-A на сайте https://www.saucedemo.com/
...
def test_sort_za(driver):
    ...
    time.sleep(3)
    za_button=driver.find_element(By.XPATH, "//option[@value='za']") # находим кнопку сортировки Z-A
    za_button.click() # кликаем по ней
    
    time.sleep(3)
    item_1 = driver.find_elements(By.CLASS_NAME, 'inventory_item_name') # находим все элементы каталога товаров     
    lst = [] # создаем пустой список, куда поместим навзание товаров в порядке их нахождения в каталоге
    for i in item_1:
        lst.append(i.text) # добавляем товары в список
    ...
    assert lst_1 == lst # проверяем равен ли список каталога отсортированному  в обратном пордяке списку
    driver.quit()
